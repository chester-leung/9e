...
function angle_between {
	...
	if [ $(float_lte $(cosine $ca) $(cosine $ba); echo $?) -eq 0 ] && [ $(float_lte $(cosine $ca) $(cosine $cb); echo $?) -eq 0 ]
	then
		return 0
	else
		return 1
...
function does_cat_see_mouse {
	...
	diff=$(bashcalc "$cat_angle - $mouse_angle")
	cos=$(cosine $diff)
	prod=$(bashcalc "$cat_radius*$cos")
	local test_result=$(float_lte 1 "$prod"; echo $?)
	if [ $test_result -eq 0 ]
	then
		return 0
	else
...
function print_status {
	...
	printf "Step $step, $state. \nCurrently,  cat_angle = %.3f; cat_radius = $old_cat_radius; mouse_angle = %.3f; max_steps= $max_steps\n\n" "$old_cat_angle" "$old_mouse_angle"
}
